...
const MAX_PACK_BYTES = 384 * 1024 * 1024;
...
function downloadBaseName(value, fallback = 'Android App') {
  const out = String(value || '')
    .normalize('NFKD')
    .replace(/[<>:\"/\|?*\x00-\x1F]+/g, '')
    .replace(/\s+/g, ' ')
...
async function iconDownload(row) {
  ...
  return {
    buffer: image.buffer,
    mime: image.mime,
    filename: `${downloadBaseName(row.name)} Icon.${image.ext}`
  };
}
...
async function coverDownload(row) {
  ...
  return { buffer:image.buffer, mime:image.mime, filename:`${downloadBaseName(row.name)} Cover.${image.ext}` };
}
...
async function screenshotDownload(row, index) {
  ...
  return {
    buffer: image.buffer,
    mime: image.mime,
    filename: `${downloadBaseName(row.name)} Screenshot ${n + 1}.${image.ext}`
  };
}
...
async function mediaPack(row) {
  ...
  for (const target of targets) {
    try {
      ...
      const base = downloadBaseName(row.name);
      const name = target.kind === 'icon'
        ? `${base} Icon.${image.ext}`
        : target.kind === 'cover' ? `${base} Cover.${image.ext}` : `${base} Screenshot ${target.index + 1}.${image.ext}`;
  ...
  return {
    buffer: zip,
    filename: `${downloadBaseName(row.name)} Media.zip`,
