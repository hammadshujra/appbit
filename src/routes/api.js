...
router.get('/r2/tree',requireAdmin,async(req,res,next)=>{try{res.json({ok:true,accounts:await r2.folderTree()})}catch(e){next(e)}});
router.get('/r2/folders/icon',requireAdmin,async(req,res,next)=>{try{const icon=await r2.getFolderIcon(req.query.accountId,req.query.key);if(!icon)return res.status(404).end();res.setHeader('Content-Type',icon.mime);res.setHeader('Cache-Control','private, max-age=3600');res.send(icon.buffer)}catch(e){next(e)}});
router.post('/r2/folders',...mutate,requireAdmin,async(req,res,next)=>{try{const folder=await r2.createFolder(req.body?.accountId,{prefix:req.body?.prefix,name:req.body?.name,iconDataUrl:req.body?.iconDataUrl});await activity.record(req.currentUser.id,'r2_folder_created',{details:{accountId:folder.accountId,key:folder.key,hasIcon:Boolean(folder.hasIcon)}});res.json({ok:true,folder})}catch(e){next(e)}});
router.get('/r2/objects',requireAdmin,async(req,res,next)=>{try{res.json({ok:true,...await r2.listObjects({accountId:req.query.accountId,q:req.query.q,prefix:req.query.prefix,limit:req.query.limit,offset:req.query.offset})})}catch(e){next(e)}});
