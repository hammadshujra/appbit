...
router.post('/r2/folders',...mutate,requireAdmin,async(req,res,next)=>{try{const folder=await r2.createFolder(req.body?.accountId,{prefix:req.body?.prefix,name:req.body?.name});await activity.record(req.currentUser.id,'r2_folder_created',{details:{accountId:folder.accountId,key:folder.key}});res.json({ok:true,folder})}catch(e){next(e)}});
