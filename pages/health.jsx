...
export default function HealthPage(){
  ...
  return <><Head><title>System health · Happy Cloud</title><meta name="viewport" content="width=device-width,initial-scale=1"/><link rel="icon" href="/favicon.svg?v=260" type="image/svg+xml"/><link rel="preconnect" href="https://fonts.googleapis.com"/><link rel="preconnect" href="https://fonts.gstatic.com" crossOrigin="anonymous"/><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet"/><link rel="stylesheet" href="/css/app.css?v=260"/></Head>
  <main className="health-split-page">
    <section className="health-visual-panel">
      <div className="auth-brand"><img src="/studio-logo.svg?v=260" alt="Happy Cloud"/><div><b>Happy Cloud</b><span>Studio Manager</span></div></div>
      <div className="health-visual-copy"><span className="auth-eyebrow">DEPLOYMENT STATUS</span><h1>{healthy?'Everything is ready.':'Checking your studio.'}</h1><p>Private deployment diagnostics for Happy Cloud on Hostinger. This page verifies runtime, database, schema, and the public download gateway without exposing credentials.</p><div className={`health-live-badge ${healthy?'ok':''}`}><i></i><span>{healthy?'All core systems operational':'Health checks running…'}</span></div></div>
    ...
    <section className="health-panel-wrap"><div className="health-panel-card">
      ...
      <div className="health-actions"><button className="btn btn-primary" type="button" onClick={refresh}>{loading?'Checking…':'Check again'}</button><a className="btn health-secondary" href="/login?next=%2F">Open sign in</a>{healthy?<a className="btn health-secondary" href="/">Open Happy Cloud</a>:null}</div>
