...
test('V2.11 release identity is bundled and never reads loose VERSION at runtime',()=>{
  const version=read('src/version.js');
  assert.match(version,/version:'2\.11'/);
  assert.match(version,/buildId:'2\.11-hostinger-native-next-runtime'/);
  assert.doesNotMatch(version,/readFileSync|node:fs|BUILD-INFO\.json|path\.join/);
  const page=read('pages/[[...path]].jsx');
  assert.match(page,/APP_VERSION='2\.11'/);
...
test('Hostinger Next preset starts native Next and has an API catch-all around the existing Appbit backend',()=>{
  ...
  assert.match(route,/responseLimit:false/);
