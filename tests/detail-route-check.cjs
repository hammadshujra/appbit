...
test('the Next.js production shell keeps canonical app and library routes',()=>{
 const server=fs.readFileSync(path.join(root,'server.js'),'utf8');
 const page=fs.readFileSync(path.join(root,'pages','[[...path]].jsx'),'utf8');
 assert.ok(server.includes('function isAllowedUiPath')&&server.includes('.test(pathname)'));
 for(const route of ["'/library'","'/library/add'","'/publishing'","'/updates'","'/r2/accounts'","'/r2/files'","'/settings'"])assert.ok(server.includes(route),route);
 assert.match(page,/\/ui\/app\.js/);
 assert.match(page,/Loading Appbit/);
});
function mockApi(){
 const routes=[];let selected=null;
 const noop=(req,res,next)=>next?.();
 const router={use(){},get(path,...handlers){routes.push({method:'GET',path,handler:handlers.at(-1)})},post(path,...handlers){routes.push({method:'POST',path,handler:handlers.at(-1)})},put(path,...handlers){routes.push({method:'PUT',path,handler:handlers.at(-1)})},delete(path,...handlers){routes.push({method:'DELETE',path,handler:handlers.at(-1)})}};
 ...
 const stub=id=>{
  ...
  if(id==='../services/apk-history')return require('../src/services/apk-history');
  if(id==='../services/apk-fields')return require('../src/services/apk-fields');
