...
test('V2.13 has exactly one Hostinger API catch-all and no legacy module route',()=>{
...
test('prebuild guard rejects stale or duplicate API bridge layouts',()=>{
  const guard=read('scripts/hostinger-prebuild.js');
  assert.match(guard,/\[\[\.\.\.path\]\]\.js/);
  assert.match(guard,/\[\.\.\.path\]\.mjs/);
  assert.match(guard,/legacy\/conflicting API route/);
