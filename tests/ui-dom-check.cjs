...
const calls=[];const record={slug:'binance-7',url:'/apps/binance-7',id:7,name:'Binance',packageId:'liteapks:binance',sourcePackageId:'com.binance.dev',sourcePageUrl:'https://liteapks.com/binance.html',sourceUpdatedAt:'2026-09-01',sourceMeta:{version:'3.19.5',downloadCount:1200000,downloadCountLabel:'1.2M+',updatedDate:'2026-09-01',fileSizeBytes:10000000,descriptionSectionText:'Exact description to copy.',sourceContentText:'Exact description to copy.',metaTitle:'Binance APK',metaDescription:'Binance source description.',historyPageUrl:'https://liteapks.com/old-versions/binance',oldVersions:[{version:'3.18.0',sourcePageUrl:'https://liteapks.com/download/binance-v3.18.0'}],iconUrl:'https://images.example/icon.png',coverImageUrl:'https://images.example/cover.png',screenshots:['https://images.example/s1.png'],playStoreUrl:'https://play.google.com/store/apps/details?id=com.binance.dev'},media:{iconUrl:'https://images.example/icon.png',coverImageUrl:'https://images.example/cover.png',screenshots:['https://images.example/s1.png']},developer:'Example',version:'3.19.5',category:'Finance',sourceSection:'apps',ratingValue:3.4,description:'Exact description to copy.',officialUrl:'https://play.google.com/store/apps/details?id=com.binance.dev',apkType:'Free',fileSizeBytes:10000000,downloadCount:1200000,published:false,versions:[{version:'3.18.0',sourcePageUrl:'https://liteapks.com/download/binance-v3.18.0'}]};
...
function api(url,opts={}){const u=new URL(url,'https://appbit.test'),body=opts.body?JSON.parse(opts.body):{};calls.push({path:u.pathname,query:u.searchParams,body,method:opts.method||'GET'});
 switch(u.pathname){
 case '/api/bootstrap':return {version:'2.0.0',buildId:'2.0.0-import-layout',csrfToken:'mock-token',user:{id:1,name:'Administrator',email:'admin@appbit.local',role:'admin'},counts,notificationSummary:{unreadCount:0}};
...
test('App Library preserves1853 count and requests32 per page',async()=>{UI.shell();await UI.go('library',{history:false});assert.match(html(),/1,853/);assert.match(html(),/32 apps per page/);assert.equal((html().match(/class="app-card(?: |")/g)||[]).length,32);assert.equal(calls.filter(c=>c.path==='/api/apps').at(-1).query.get('perPage'),'32')});
...
test('detail keeps source links, one description, and excludes technical/source values from Copy All',async()=>{
 await UI.openDetail(7);assert.equal(location.pathname,'/apps/binance-7');const page=html();
 assert.match(page,/app-record-page/);assert.doesNotMatch(page,/class="modal/);
 assert.match(page,/Update Date/);assert.match(page,/App Size/);assert.match(page,/1.2M\+/);
 assert.match(page,/play\.google\.com\/store\/apps\/details/);
 assert.match(page,/href="https:\/\/liteapks\.com\/binance\.html"/);
 assert.match(page,/href="https:\/\/liteapks\.com\/download\/binance-v3\.18\.0"/);
 assert.equal((page.match(/<h2>Core Content<\/h2>/g)||[]).length,1);
 assert.equal((page.match(/Download Media<\/a>/g)||[]).length,1);
 assert.ok(page.indexOf('APK Metadata')<page.indexOf('<h2>Core Content'));
 assert.ok(page.indexOf('<h2>Links')<page.indexOf('<h2>Core Content'));
 assert.doesNotMatch(page,/record-version-preview|id="refreshLogo"|id="refreshScreenshot"/);
 const copied=nodes.get('copyAppAll').dataset.copy;
 assert.match(copied,/Official Play Store:/);assert.match(copied,/Exact description to copy\./);
 assert.doesNotMatch(copied,/liteapks:binance|liteapks\.com|Package ID|Source Views/);
 assert.match(nodes.get('app').innerHTML,/V2\.0\.0/);
 await UI.releaseActiveDetail();
});
test('unified Media Refresh scans icon, cover and screenshots only for selected app',async()=>{
 await UI.openDetail(7);await nodes.get('refreshMedia').onclick();
 assert.ok(calls.some(c=>c.path==='/api/apps/7/media/refresh'&&c.method==='POST'&&JSON.stringify(c.body.kinds)==='["icon","cover","screenshot"]'));
 assert.ok(!calls.some(c=>c.path==='/api/updates/start'));
 await UI.releaseActiveDetail();
});
test('Add APK opens inline and submits the supplied URL',async()=>{
 await UI.go('library');assert.match(html(),/id="inlineImportForm"/);
 assert.equal(location.pathname,'/library');assert.match(html(),/id="inlineImportForm"/);
 nodes.get('sourceUrl').value='https://liteapks.com/binance.html';
 await nodes.get('inlineImportForm').onsubmit({preventDefault(){}});
 assert.ok(calls.some(c=>c.path==='/api/apps/import'&&c.body.sourcePageUrl==='https://liteapks.com/binance.html'));
 assert.equal(location.pathname,'/apps/binance-7');await UI.releaseActiveDetail();
});
test('manual batch start/stop remains explicit and never runs on library load',async()=>{
 await UI.go('library');assert.ok(!calls.some(c=>c.path==='/api/resolver/sync'));
 nodes.get('content').innerHTML='<input id="syncLimit" value="5"><select id="syncMode" value="new"></select><button id="syncNow"></button><button id="stopSync"></button><div id="importStatus"></div>';
 nodes.get('syncLimit').value='5';nodes.get('syncMode').value='new';
 await UI.startManualImport();assert.ok(calls.some(c=>c.path==='/api/resolver/sync'&&c.body.limit===5));
 await UI.stopManualImport();assert.ok(calls.some(c=>c.path==='/api/resolver/stop'));
});
