...
function loadModule(rel,stubs={},hook=''){
 ...
 const context={module,exports:module.exports,require:id=>id==='../utils/sql-checked'?require('../src/utils/sql-checked'):stubs[id]??require(id),Buffer,URL,URLSearchParams,AbortController,Response,Headers,fetch,console,setTimeout,clearTimeout,setInterval,clearInterval,Date,Math,Number,JSON,String,Promise,Array,Map,Set,RegExp,process,crypto:require('node:crypto')};
...
function resolverHarness(pool,hook=''){
 ...
 const stubs={'../db':{getPool:()=>pool},'../state':state,'../config':{apkResolverMaxItems:50000,apkResolverRequestGapMs:700},'./activity':{record:async()=>{}},'./notifications':{notifyUser:async()=>{}},'../utils/version':{compareVersions:(a,b)=>String(a).localeCompare(String(b),undefined,{numeric:true})},'./apk-source':require('../src/services/apk-source'),'./apk-fields':require('../src/services/apk-fields'),'./apk-history':require('../src/services/apk-history'),'./remote':{probeRemoteFileSize:async()=>null},'cheerio':{load:()=>({})}};
...
test('403/challenge use one browser fallback; 429 remains bounded',async()=>{
 const source=require('../src/services/apk-source');let calls=0,browserCalls=0;
 const denied=async()=>{browserCalls++;throw new source.SourceError('browser denied',403,'SOURCE_ACCESS_DENIED')};
 await assert.rejects(source.fetchText('https://liteapks.com/a',{gapMs:1,fetchImpl:async()=>{calls++;return new Response('blocked',{status:403})},browserFallbackImpl:denied}),e=>e.code==='SOURCE_ACCESS_DENIED'&&e.status===403);assert.equal(calls,1);assert.equal(browserCalls,1);
 browserCalls=0;await assert.rejects(source.fetchText('https://liteapks.com/a',{gapMs:1,fetchImpl:async()=>new Response('<title>Just a moment...</title><div class="cf-turnstile"></div>'),browserFallbackImpl:async()=>{browserCalls++;throw new source.SourceError('browser challenge',403,'SOURCE_CHALLENGE')}}),e=>e.code==='SOURCE_CHALLENGE');assert.equal(browserCalls,1);
 calls=0;await assert.rejects(source.fetchText('https://liteapks.com/a',{gapMs:1,fetchImpl:async()=>{calls++;return new Response('slow down',{status:429,headers:{'retry-after':'0'}})},browserFallbackImpl:denied}),e=>e.code==='SOURCE_RATE_LIMITED');assert.equal(calls,3);
});
...
test('manual worker skips one 403 app and continues the remaining queue',async()=>{
 ...
 await h.exports.startSync({mode:'full',limit:2});await flush();await h.exports.step();let st=await h.exports.getSyncState();assert.equal(st.status,'running');assert.equal(JSON.parse(st.queue_json).length,1);assert.equal(st.processed_count,1);assert.equal(st.failed_count,1);await h.exports.step();st=await h.exports.getSyncState();assert.equal(st.status,'complete');assert.equal(fetchCount,2);assert.equal(st.failed_count,2);
});
