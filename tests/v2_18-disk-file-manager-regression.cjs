...
test('V2.19 removes the hidden R2 root-folder setting and treats account label as disk metadata only',()=>{
...
test('V2.19 file manager opens disks and uses current-folder explorer semantics',()=>{
...
test('V2.19 supports Windows drag-drop multi-upload with per-file progress rows',()=>{
...
test('V2.19 explorer shows requested metadata and file actions',()=>{
...
test('direct public URL format is exactly domain/filename and folders are virtual only',()=>{
