...
test('migration repairs before category initialization and never re-enters legacy deletion for existing apps',async()=>{
  ...
  const events=[];const appColumns=['id','package_id','name','source_page_url'].map(COLUMN_NAME=>({COLUMN_NAME}));
  ...
  assert.equal(await module.exports.migrate(),132);
...
test('accepts a valid existing apps table with no migration marker, but rejects a future marker',async()=>{
  const file=fs.readFileSync(path.join(root,'src/migrations.js'),'utf8');
  const columns=['id','package_id','name','source_page_url'].map(COLUMN_NAME=>({COLUMN_NAME}));
  ...
  const future=createDb(133),module2={exports:{}};
